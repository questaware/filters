...
NEAR setekey(key)   /* reset encryption key of current buffer */
	char *  key;
{
   initcrypt(key, strlen(key));
}
...
static long key_ = 0;	/* 29 bit encipherment key */
...
PASCAL NEAR initcrypt(bptr, len)
	register char *bptr;	/* buffer of characters to be encrypted */
	register unsigned len;	/* number of characters in the buffer */
